...
현재 저장소에는 1단계 프런트와 2단계 서버 인터페이스·마이그레이션 초안이 구현되어 있습니다. Supabase, Cloudflare, 카카오 알림 리소스는 아직 만들거나 연결하지 않았습니다. 따라서 핵심 행사 정보는 동작하지만 질문 폼은 입력 내용을 외부로 보내지 않고 “연결 준비 중”이라고 명확히 안내합니다.
